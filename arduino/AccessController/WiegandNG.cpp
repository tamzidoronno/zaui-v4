...
bool WiegandNG::begin(uint8_t pinD0, uint8_t pinD1, unsigned int allocateBits, unsigned int packetGap) {
	...
	pinMode(pinD1, INPUT);
	attachInterupts(pinD0, pinD1);
	return true;
}

void WiegandNG::attachInterupts(uint8_t pinD0, uint8_t pinD1) {
...
void WiegandNG::detachInterrupts(uint8_t pinD0, uint8_t pinD1) {
	detachInterrupt(digitalPinToInterrupt(pinD0));			// hardware interrupt - high to low pulse
	detachInterrupt(digitalPinToInterrupt(pinD1));			// hardware interrupt - high to low pulse
}
