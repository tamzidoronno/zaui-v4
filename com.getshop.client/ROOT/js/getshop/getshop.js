...
thundashop.framework = {
    ...
    loadAppInOverLay: function(appId, overlayid, data) {
 
        
        getshop.showOverlay(overlayid);
        
        var div = $('<div/>');
        div.addClass('app');
        div.attr('appid', appId);
        div.attr('appid2', appId);
        div.attr('appsettingsid', appId);
        
        var button = $('<div></div>');
        div.append(button);
        
        var event = thundashop.Ajax.createEvent(null, "renderStandAlone", button, data);
        event['synchron'] = true;
        
        thundashop.Ajax.post(event, function(res) {
            $('.gsoverlay'+overlayid+' .gsoverlayinner .content').html(res);
        });
    }
}
