<?
include '../loader.php';
$pageFactory = new \PageFactory("pmsconference");
...
if(!$factory->getApi()->getUserManager()->isLoggedIn() || !ns_df435931_9364_4b6a_b4b2_951c90cc0d70\Login::getUserObject()) {
    setcookie('PHPSESSID','');
    header('location:/login.php?redirectto=/pmsconference.php');
    exit(0);
}
...
?>
<html pageid="<? echo $page->getId(); ?>" module="pmsconference">
    <head>
        <title>Conference - GetShop</title>
        ...
        <link rel="stylesheet" href="/skin/default/pmsconference.css?<? echo calculateCacheName(); ?>">
    ...
    <body>
        ...
        <div area="header" class="gsarea">
            <?
            $menu = $page->getMenu("pmsconference");
        ...
        <div class="gs_body" hasleftmenu="<? echo $page->getLeftMenu() ? 'yes' : 'no'; ?>">
            <?
            if ($page->getLeftMenu()) { $page->getLeftMenu()->renderLeft("pmsconference"); }
            ?>
