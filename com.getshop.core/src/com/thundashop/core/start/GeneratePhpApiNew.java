...
public class GeneratePhpApiNew {
    ...
    private static void buildDocumentation(List<Class> list) throws IOException, ClassNotFoundException {
        ...
        for(String name : storedClassesStrings) {
            ...
            for(Method method : sortedMethods) {
                Map<String, Object> parsedMethod = parseMethod(path, method);
            ...
            for (Annotation annotation : annotations) {
                if (annotation instanceof GetShopApi) {
                    ...
                    for (Method method : methods) {
                        String path = entry.getCanonicalName();
                        path = path.replace(".", "/") + ".java";
                        Map<String, Object> doc = parseMethod(path, method);
    ...
    private static Map<String, Object> parseMethod(String path, Method method) {
        String content = readFile(path, true);

        List<String> splittedArgs = new ArrayList();
        String newArgs = createArgumentString(content, method, splittedArgs);

        newArgs = newArgs.trim();
        String[] commentLines = createCommentLines(content, content.indexOf(method.getName() + "("));
        String returnvalue = createReturnValue(method);
    ...
    private static String createReturnValue(Method method) {
    ...
    private static String createArgumentString(String content, Method method, List<String> splittedArgs) {
