 ...
 */
@Component
public class PmsChangeDefaultPayLaterSettings extends UpdateScriptBase implements UpdateScript {

    @Override
    public Date getAddedDate() {
        return getDate("18/12-2018");
    }
    
    @Override
    public String getId() {
        return "108d370e-a56c-4770-ad7c-5ebce41a0b6d";
    }
    
    public static void main(String[] args) {
        new PmsChangeDefaultPayLaterSettings().runSingle();
    }
            ...
            for(String name : names) {
                ...
                for(DataCommon tmp : res) {
                    if(tmp instanceof PmsConfiguration) {
                        ...
                        if (config.ignorePaymentWindowDaysAheadOfStay == 0) {
                            config.ignorePaymentWindowDaysAheadOfStay = -1;
                            save = true;
                        }
                        if(save) {
