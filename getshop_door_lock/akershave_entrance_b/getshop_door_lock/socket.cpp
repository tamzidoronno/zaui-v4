...
int client_socket[30], client_socket2[30], client_socket3[30], max_clients = 30;
...
void callback3(int bits, uint32_t value) {
   printf("Got: %d", value);
   for (int i=0; i<max_clients;i++) {
      sendData( client_socket3[i], value );
   }
}
...
int main(int argc, char *argv[]) {
    ...
    master_socket3 = createMasterSocket(address_51519, 51519);
    
    //accept the incoming connection
    addrlen = sizeof (address_51517);
    addrlen2 = sizeof (address_51518);
    addrlen3 = sizeof (address_51519);
    ...
    while (TRUE) {
        ...
        if (FD_ISSET(master_socket3, &readfds)) {
            createNewSocket(master_socket3, address_51519, addrlen3, max_clients, client_socket3);
        }
        ...
        for (int i = 0; i < max_clients; i++) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                // Handle incoming message
                handleSocket(address_51519, addrlen3, sd, client_socket3, i);
            }
        }
    }
