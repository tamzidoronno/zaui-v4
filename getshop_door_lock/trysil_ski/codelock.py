...
class DoorLock(object):
     ...
     def checkPinCode(self, code):
        ...
        f = open('/storage/forcecode.txt', 'r')
        try:
             forceCodes = f.read()
        finally:
             f.close();
        ...
        if code in forceCodes:
             if self.FORCEDOPEN:
                 self.writeToForceState("off");
             else:
                 self.writeToForceState("on");
 
     def writeToForceState(self, state):
         fileName = "/storage/forceopenstate"+str(self.DEVICEID);
         f = open(fileName, "w");
     ...
     def isForcedOpen(self):
         fileName = "/storage/forceopenstate";
     ...
     def checkForceState(self):
         ...
         while True:
             ...
             # Retrigger the light to prevent it to turn to red.
             # Needs to be done for the metal case 
             if self.FORCEDOPEN:
                  if COUNTER > 60:
                       COUNTER = 0;
