...
controllers.DestinationController = function($scope, datarepository, $stateParams, $api, $state) {
    ...
    $scope.resendData = function() {
        if (!messagePersister) {
            alert("Not working locally");
        }
        
        var res = prompt("Warning, are you sure you want to resend data. Type yes to continue");
        if (res === "yes") {
            messagePersister.get1000LatestMessages($scope.gotData, $api);
        }
    }
    
    $scope.gotData = function(messages, api) {
        ...
        for (var i in $scope.destination.tasks) {
            var task = $scope.destination.tasks[i];
            taskIds.push(JSON.stringify(task.id));
        }
        ...
        for (var i in sortedMessages) {
            ...
            if (msg.method === "markAsArrived" && JSON.stringify($scope.destination.id) === msg.args.destinationId) {
                api.getApi().send(msg);
                messageCount++;
            }
            
            if (msg.method === "markInstructionAsRead" && JSON.stringify($scope.destination.id) === msg.args.destinationId) {
                api.getApi().send(msg);
                messageCount++;
            }
            
            if (msg.method === "replyMessageForDestionation" && JSON.stringify($scope.destination.id) === msg.args.destinationId) {
                api.getApi().send(msg);
                messageCount++;
            }
            
            if (msg.method === "markDeparting" && JSON.stringify($scope.destination.id) === msg.args.destinationId) {
                api.getApi().send(msg);
                messageCount++;
            }
            
            if (msg.method === "unsetSkippedReason" && JSON.stringify($scope.destination.id) === msg.args.destinationId) {
                api.getApi().send(msg);
                messageCount++;
            }
            
            if (msg.method === "addPickupOrder" && JSON.stringify($scope.destination.id) === msg.args.destinationId) {
                api.getApi().send(msg);
                messageCount++;
            }
            
            // Task api calls
            if (msg.method === "markAsDeliverd" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "markTaskWithExceptionDeliverd" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "markOrderWithException" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "changeQuantity" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "setCagesOrPalletCount" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "changeQuantity" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "changeCountedDriverCopies" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
            if (msg.method === "markOrderWithException" && taskIds.indexOf(msg.args.taskId) > -1) { api.getApi().send(msg); messageCount++; }
        }
        
        alert("Resent "+ messageCount + " messages.");
    }
}