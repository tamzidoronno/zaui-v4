...
; (function ($, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        type: function (key) {
            var input = pga.lastSelectedInput;
            input.find('.inner').html(input.find('.inner').html() + key);
            input.removeClass('error');
            pga.lastSelectedInput.focus();
        },

        backspace: function () {
            var input = pga.lastSelectedInput;
            var val = input.find('.inner').html();

            input.find('.inner').html(val.substr(0, val.length - 1));
        },
