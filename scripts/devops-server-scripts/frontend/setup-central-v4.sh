...
echo "Unzipping";
tar zxvf ../php_central_4.1.0.tar.gz > /dev/null 2>&1;
...
chmod 777 app;
chmod 777 etc;
echo "backenddb=10.0.8.73" > etc/config.txt
echo "port=35555" >> etc/config.txt
...
#echo "Adding partners applications (not saure if we really need that here)";
#cd app;
...
sudo rm -rf www;
mv tmp www;
rm -rf php_central_4.1.0.tar.gz
