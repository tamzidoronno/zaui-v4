...
public class GotoBookingRequestValidationService implements IGotoBookingRequestValidationService{
    ...
    public void validateSaveBookingDto(GotoBookingRequest bookingRequest,
        ...
        for(GotoRoomRequest room : bookingRequest.getRooms()) {
            BookingItemType type = bookingItemTypeService.getBookingItemTypeById(room.getRoomCode(), bookingItemTypeSession);
            try{
                ...
                e.setMessage(e.getMessage()+", RoomCode: " + room.getRoomCode() + ((type == null) ? "" : ", RoomType: " + type.name));
                throw e;
            }
        }
    }
    ...
    private void validateBookerInfo(GotoBooker booker) throws GotoException {
        if(isBlank(booker.getEmail()) && (
                booker.getMobile() == null || isBlank(booker.getMobile().getAreaCode()) || isBlank(booker.getMobile().getPhoneNumber()))
    ...
    private void validatePrice(String checkIn, String checkOut, GotoRoomPrice price) throws GotoException, ParseException {
        if(price == null || price.getDailyPrices() == null || price.getDailyPrices().isEmpty() || price.getTotalRoomPrice() == null) {
        ...
        while(!calendar.getTime().after(checkOutDate)) {
            ...
            if(price == null || roomPrice < 0) throw new GotoException(PRICE_MISSING.code, PRICE_MISSING.message);
